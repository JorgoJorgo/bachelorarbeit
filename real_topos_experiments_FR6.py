...
'SquareOne FR6': [PrepareSQ1, RouteSQ1],
'MultipleTrees Mod Parallel': [multiple_trees_pre_parallel, RouteMultipleTrees],
'MultipleTrees Invert Order Mod FR6': [multiple_trees_pre_invert_order_of_edps_mod, RouteMultipleTrees],
'MultipleTrees Random Order Mod FR6': [multiple_trees_pre_random_order_of_edps_mod, RouteMultipleTrees]
}
...
def run_zoo(out=None, seed=0, rep=2):
    global f_num
    fr = 6 #die zahl muss geändert werden damit man die fr ändert
    min_connectivity = 2
    original_params = [n, rep, k, samplesize, f_num, seed, name]
    if DEBUG:
...
def experiments(switch="all", seed=0, rep=100):
    ...
    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-RealTopos-FR6-" + str(k))
