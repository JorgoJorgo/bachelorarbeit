...
algos = {'Greedy FR9': [GreedyArborescenceDecomposition, RouteDetCircNotSpanning],
    'MultipleTrees FR9': [multiple_trees_pre, RouteMultipleTrees],
'OneTree FR9': [one_tree_pre, RouteOneTree],
'Parallel and Inverse FR9': [multiple_trees_pre_parallel_and_inverse, RouteMultipleTrees],
'Breite and Inverse FR9': [multiple_trees_pre_breite_mod_and_inverse, RouteMultipleTrees],
'Anzahl and Random FR9' : [multiple_trees_pre_num_of_trees_mod_and_random_order, RouteMultipleTrees]
}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-combination-FR9" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-combination-FR9" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_-combination-FR9" + str(seed))
