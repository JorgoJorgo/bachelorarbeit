...
algos = {'MultipleTrees Base FR5': [multiple_trees_pre,RouteMultipleTrees],'MultipleTrees Random Order Mod FR5': [multiple_trees_pre_random_order_of_edps_mod, RouteMultipleTrees]}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-FR5-MTvsMTORM-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-FR5-MTvsMTORM-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_FR5-MTvsMTORM-" + str(seed))
