...
algos = {'MultipleTrees Base FR6': [multiple_trees_pre,RouteMultipleTrees],'MultipleTrees Invert Order Mod FR6': [multiple_trees_pre_invert_order_of_edps_mod, RouteMultipleTrees]}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-FR6-MTvsMTOIM-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-FR6-MTvsMTOIM-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_FR6-MTvsMTOIM-" + str(seed))
