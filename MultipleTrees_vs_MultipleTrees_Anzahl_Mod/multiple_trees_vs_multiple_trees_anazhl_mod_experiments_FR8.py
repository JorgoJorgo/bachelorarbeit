...
algos = {'MultipleTrees Base FR8': [multiple_trees_pre,RouteMultipleTrees],'Multiple Trees Anzahl Mod FR8': [multiple_trees_pre_num_of_trees_mod,RouteMultipleTrees]}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-FR8-MTvsMTAM-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-FR8-MTvsMTAM-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_FR8-MTvsMTAM-" + str(seed))
