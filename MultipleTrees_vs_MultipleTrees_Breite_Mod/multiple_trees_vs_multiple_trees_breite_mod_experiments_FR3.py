...
algos = {'MultipleTrees Base FR3': [multiple_trees_pre,RouteMultipleTrees],'Multiple Trees Breite Mod FR3': [multiple_trees_pre_breite_mod,RouteMultipleTrees]}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-FR3-MTvsMTBM-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-FR3-MTvsMTBM-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_FR3-MTvsMTBM-" + str(seed))
