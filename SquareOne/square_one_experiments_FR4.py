...
from routing import RouteMultipleTrees, PrepareSQ1 , RouteSQ1, returnSQ1
DEBUG = True
...
algos = {'SquareOne FR4': [PrepareSQ1, RouteSQ1]}
...
def one_experiment(g, seed, out, algo):
    ...
    #PrepareSQ1(g)
    precomputation = precomputation_algo(g)
    #print("Precomputation : " , precomputation)
...
def run_regular(out=None, seed=0, rep=5):
    ...
    for i in range(rep):
        ...
        for (algoname, algo) in algos.items():
            ...
            algos[algoname] += [one_experiment(g, seed + i, out, algo)]#hier findet der aufruf statt den algorithmus auszuführen
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-sq1-FR4-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-sq1-FR4-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_-sq1-FR4-" + str(seed))
