...
algos = {'MultipleTrees Base FR5': [multiple_trees_pre,RouteMultipleTrees],'Multiple Trees Parallel Mod FR5': [multiple_trees_pre_parallel, RouteMultipleTrees]}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-FR5-MTvsMTPM-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-FR5-MTvsMTPM-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_FR5-MTvsMTPM-" + str(seed))
