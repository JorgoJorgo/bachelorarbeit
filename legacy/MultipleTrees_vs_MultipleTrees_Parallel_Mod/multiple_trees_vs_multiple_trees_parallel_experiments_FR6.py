...
algos = {'MultipleTrees Base FR6': [multiple_trees_pre,RouteMultipleTrees],'Multiple Trees Parallel Mod FR6': [multiple_trees_pre_parallel, RouteMultipleTrees]}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-FR6-MTvsMTPM-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-FR6-MTvsMTPM-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_FR6-MTvsMTPM-" + str(seed))
