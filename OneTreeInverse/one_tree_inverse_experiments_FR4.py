...
from trees import one_tree_pre,one_tree_pre_mod_inverse,multiple_trees_pre, multiple_trees_pre_breite_mod, multiple_trees_pre_num_of_trees_mod, multiple_trees_pre_order_of_edps_mod, multiple_trees_pre_parallel, multiple_trees_pre_recycling, one_tree_pre_breite_mod
...
'MultipleTrees FR4': [multiple_trees_pre, RouteMultipleTrees],
...
'OneTree Inverse Mod FR4': [one_tree_pre_mod_inverse,RouteOneTree],
'OneTree FR4': [one_tree_pre,RouteOneTree]
}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-inverseOneTree-FR4-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-inverseOneTree-FR4-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_-inverseOneTree-FR4-" + str(seed))
...
if __name__ == "__main__":
    f_num = 20 #number of failed links
    ...
    seed = 0  #random seed
    name = "benchmark-FR4-" #result files start with this name
