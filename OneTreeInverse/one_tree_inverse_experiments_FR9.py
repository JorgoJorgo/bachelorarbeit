...
'MultipleTrees FR9': [multiple_trees_pre, RouteMultipleTrees],
...
'OneTree Inverse Mod FR9': [one_tree_pre_mod_inverse,RouteOneTree],
'OneTree FR9': [one_tree_pre,RouteOneTree]
}
...
def experiments(switch="all", seed=0, rep=100):
    if switch in ["regular", "all"]:
        out = start_file("results/benchmark-regular-inverseOneTree-FR9-" + str(n) + "-" + str(k))
        run_regular(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["zoo", "all"]:
        out = start_file("results/benchmark-zoo-inverseOneTree-FR9-" + str(k))
        run_zoo(out=out, seed=seed, rep=rep)
        out.close()

    if switch in ["AS"]:
        out = start_file("results/benchmark-as_seed_-inverseOneTree-FR9-" + str(seed))
...
if __name__ == "__main__":
    f_num = 45 #number of failed links
    ...
    seed = 0  #random seed
    name = "benchmark-FR9-" #result files start with this name
