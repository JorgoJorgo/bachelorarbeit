...
def experiment_objective_subset(obj_func, method, objstr=None, seed=11, gml=False, torus=False):
    if objstr == None:
        objstr = str(obj_func)
    random.seed(seed)
    filename = "results/srds-failure_objective_" + \
               str(n)+"_"+str(k)+"_"+str(seed)+"_"+objstr+".txt"
    if gml:
        filename = "results/srds-gml_failure_objective_" + \
                   str(n)+"_"+str(k)+"_"+str(seed)+"_"+objstr+".txt"
    if torus:
        filename = "results/srds-torus_failure_objective_" + \
                   str(n)+"_"+str(k)+"_"+str(seed)+"_"+objstr+".txt"
    outstretch = open(filename, 'w')
    outstretch.write(
        "#n= %d, connectivit= %d, repetitions= %d\n" % (n, k, rep))
    outstretch.write(
        ...
        if num_complete_nodes(g) == n:
            ...
            for f1 in failure_range:
                ...
                outstretch.write("regular, before, %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n" % (
                    f1, -1*before, brs, brh, stat.load, stat.load, np.max(stat.stretch), np.mean(stat.stretch), np.max(stat.hops), np.mean(stat.hops)))
                ...
                outstretch.write("regular, after, %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n" % (
                    f1, -1*after, ars, arh, stat.load, stat.load, np.max(stat.stretch), np.mean(stat.stretch), np.max(stat.hops), np.mean(stat.hops)))
                if np.mean(stat.stretch) < 0 and stat.succ > 0:
            ...
            sys.stdout.flush()
            outstretch.flush()
    if len(data[1]['before']['succ']) > 0:
        for f in failure_range:
            brs = np.mean(data[f]['before']['succ'])
            bsh = np.mean(data[f]['after']['succ'])
            ars = np.mean(data[f]['before']['hops'])
            arh = np.mean(data[f]['after']['hops'])
            print('%d failures, avg b,a success: %.2f, %.2f, b,a hops: %.2f, %.2f' % (
                f, brs, bsh, ars, arh))
            brs = np.min(data[f]['before']['succ'])
            bsh = np.min(data[f]['after']['succ'])
            ars = np.max(data[f]['before']['hops'])
            arh = np.max(data[f]['after']['hops'])
            #print('%d failures, min before/after success, %.2f, %.2f, max before/after hops %.2f, %.2f' % (f, brs, bsh, ars, arh))
            sys.stdout.flush()
    outstretch.close()
    ...
    for j in range(rep):
        ...
        if num_complete_nodes(g) == n:
            ...
            for f in failure_range:
                ...
                outstretch.write("regular, before, %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n" % (
                    f, before, brs, brh, stat.load, stat.load, np.max(stat.stretch), np.mean(stat.stretch), np.max(stat.hops), np.mean(stat.hops)))
                ...
                #                                  alg,f, succ, switches, load      stretch,    hops
                outstretch.write("regular, after, %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n" % (
                    f, after, ars, arh, stat.load, stat.load, np.max(stat.stretch), np.mean(stat.stretch), np.max(stat.hops), np.mean(stat.hops)))
                if (np.max(stat.stretch) > n-1):
            ...
            if num_complete_nodes(g) == n:
                ...
                for (u, v) in SRLG:
                    ...
                    if index in range(k-2) and v != g.graph['root']:
                        for vv in g[u]:
                            if vv != g.graph['root'] and (u, vv) not in SRLG and (vv, u) not in SRLG and g[u][vv]['arb'] in [k-1, k-2]:
                                swap(g, u, v, u, vv)
                ...
                outstretch.write("regular, before, True, %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n" % (f, before, brs, brh, np.max(
                    stat.load), np.mean(stat.load), np.max(stat.stretch), np.mean(stat.stretch), np.max(stat.hops), np.mean(stat.hops)))
                ...
                outstretch.write("regular, before, False, %d, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n" % (f, before, bss, bsh, np.max(
                    stat.load), np.mean(stat.load), np.max(stat.stretch), np.mean(stat.stretch), np.max(stat.hops), np.mean(stat.hops)))
...
def read_zoo(j):
    ...
    degree = 3
    while nx.edge_connectivity(g2) < 4:
        g2 = trim2(g2, degree)
        if len(g2.nodes) == 0:
    ...
    if len(g2.nodes) < 10:
        return None
    #print(zoo_list[j],len(g2.nodes), len(g2.edges), nx.edge_connectivity(g2),degree)
    g = g2.to_directed()
...
def generate_trimmed_AS(conn):
    ...
    if len(files) == 0:
        ...
        sys.exit()
    for x in files:
        if 'r0' in x or 'r1' in x or 'pop' in x:
            continue
        g = nx.Graph()
